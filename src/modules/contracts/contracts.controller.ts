...
export const contractsController = {
  ...
  /** PUT /api/contracts/:id — actualizar campos editables */
  async update(req: AuthenticatedRequest, res: Response, next: NextFunction): Promise<void> {
    try {
      const {
        endDate, paymentDayOfMonth, monthlyRent, currency,
        depositAmount, depositCurrency, lateFeePercent, gracePeriodDays, notes,
      } = req.body;
      ...
      const updated = await prisma.contract.update({
        ...
        data: {
          ...
          ...(paymentDayOfMonth  !== undefined && { paymentDayOfMonth }),
          ...(monthlyRent        !== undefined && { monthlyRent }),
          ...(currency           !== undefined && { currency }),
          ...(depositAmount      !== undefined && { depositAmount }),
          ...(depositCurrency    !== undefined && { depositCurrency }),
          ...(lateFeePercent     !== undefined && { lateFeePercent }),
      ...
      res.json(successResponse(updated, 'Contrato actualizado correctamente.'));
    } catch (err) { next(err); }
  },
