...
export class CallMeBotService {
  ...
  /** Notificación de nueva nota de débito (cargo por servicio público) */
  static async sendDebitNoteNotice(params: {
    phone: string;
    apiKey: string;
    tenantName: string;
    propertyUnit: string;
    ...
    const periodo = `${MONTHS_ES[params.periodMonth]} ${params.periodYear}`;
    const SERVICE_ICONS: Record<string,string> = { AGUA:'💧', LUZ:'⚡', GAS:'🔥', INTERNET:'🌐', BASURA:'🗑️', OTRO:'📋' };
    const icono = SERVICE_ICONS[params.serviceType] || '📋';

    return this.send(params.phone, params.apiKey,
      `${icono} *Rentify App — Cargo por Servicio Público*\n\n` +
      `Hola *${params.tenantName}*, se registró un cargo adicional en tu cuenta.\n\n` +
      `📍 Unidad: ${params.propertyUnit}\n` +
      `📅 Período: ${periodo}\n` +
      `🔖 Servicio: ${params.description}\n` +
      `💰 Monto: *${formatMoney(params.amount, params.currency)}*\n` +
      (params.invoiceRef ? `🧾 Factura N°: ${params.invoiceRef}\n` : '') +
      `\nEste cargo será incluido en tu próximo cobro. Para consultas contactá a tu arrendador.`
    );
  }
}
